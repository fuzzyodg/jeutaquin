    ...
    if (items.length === 0) {
      body.innerHTML = `
        <div class="cart-empty">
          ...
          <a href="menu.html" class="btn btn--secondary" style="margin-top:15px; font-size:0.8rem; padding: 0.5rem 1rem;">Voir le menu</a>
        </div>`;
      return;
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('cart-body')?.addEventListener('click', (e) => {
      ...
      if (action === 'increase') {
        ...
        Cart.remove(name);
      } else if (btn.getAttribute('href') === 'menu.html') {
        close();
      }
    });
