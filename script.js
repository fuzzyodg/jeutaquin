...
const Cart = (function () {
  ...
  function render() {
    ...
    if (items.length === 0) {
      body.innerHTML = `
        <div class="cart-empty">
          ...
          <a href="menu.html" class="btn btn--gold mt-2" style="font-size: 0.9rem; padding: 0.5rem 1rem;">Voir le menu</a>
        </div>`;
      return;
    }
    ...
    body.innerHTML = items.map(item => `
      <div class="cart-item">
        ${item.imgSrc
          ? `<img class="cart-item__img" src="${item.imgSrc}" alt="${esc(item.name)}" onerror="this.style.display='none'">`
        ...
        <button class="cart-item__remove" data-action="remove" data-name="${esc(item.name)}" aria-label="Retirer ${esc(item.name)} du panier">🗑</button>
      </div>`).join('');
  }
