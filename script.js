...
const Cart = (function () {
  ...
  function add(name, price, imgSrc) {
    ...
    // Animation de rebond sur le bouton panier
    const cartBtn = document.getElementById('cart-btn');
    if (cartBtn) {
      cartBtn.classList.remove('cart-bump');
      void cartBtn.offsetWidth; // Force reflow pour redémarrer l'animation
  ...
  function render() {
    ...
    if (items.length === 0) {
      body.innerHTML = `
        <div class="cart-empty">
          ...
          <a href="menu.html" class="btn btn--gold mt-2" id="cart-menu-link">Voir le menu</a>
        </div>`;
      return;
    }
    ...
    body.innerHTML = items.map(item => {
      ...
      <div class="cart-item">
        ...
        <button class="cart-item__remove" data-action="remove" data-name="${escapedName}" aria-label="Supprimer ${escapedName} du panier" title="Supprimer">🗑</button>
      </div>`;
    }).join('');
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('cart-body')?.addEventListener('click', (e) => {
      ...
      else if (btn.id === 'cart-menu-link') close();
    });
