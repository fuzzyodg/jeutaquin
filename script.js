...
const Cart = (function () {
  ...
  function add(name, price, imgSrc) {
    ...
    // Feedback visuel (animation bump) sur le bouton panier
    ...
    if (items.length === 0) {
      body.innerHTML = `
        <div class="cart-empty">
          ...
          <a href="menu.html" class="cart-empty__cta" id="cart-go-menu">Voir le menu</a>
        </div>`;
      return;
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (body) {
      body.addEventListener('click', (e) => {
        ...
        if (!btn) {
          // Gestion du lien "Voir le menu" dans le panier vide
    ...
    // ARIA label pour le bouton de fermeture
    const closeBtn = document.getElementById('cart-close');
    if (closeBtn) closeBtn.setAttribute('aria-label', 'Fermer le panier');
